...
CSV 格式假定每行第一列是字符串（图像文件名或路径），其余列是浮点数（图像特征值）。
读函数把文件名存为 std::vector<char*>，把特征值存为 std::vector<std::vector<float>>；
写函数把单张图像的特征追加到 CSV 文件中
读：调用 read_image_data_csv("features.csv", filenames, data, 1) 后，filenames[i] 对应 data[i]。
写：每计算出一张图像的特征向量后，调用 append_image_data_csv("features.csv", "img.jpg", feat, 0) 将一行追加到文件
*/
