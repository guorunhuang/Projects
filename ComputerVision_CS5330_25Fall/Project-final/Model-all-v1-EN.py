# compare to the Model-all-v1.py:
# 所有中文内容已替换为英文，包括注释和打印语句
# 删除了 plot_feature_importance() 函数 - 不再需要
# 打印所有特征重要性分数 - 现在在训练循环中，它会使用以下代码打印包含所有特征及其重要性分数的完整排序 DataFrame：

# python   print(f"\n所有特征重要性分数：")
# print(importance_df.to_string(index=False))
# 现在，代码会在训练期间输出每个目标的完整特征重要性表格，显示每个特征的分数，并按从高到低的顺序排列。
# 这使得您可以轻松查看哪些特征对每个生物量预测目标影响最大。
import pandas as pd
import numpy as np
import cv2
