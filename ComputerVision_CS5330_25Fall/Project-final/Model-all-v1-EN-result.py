'''
comment:是不是重要性小于0.02的其实都可以省略了，不再把这些特征作为模型的输入？
尝试特征选择, 可以先设一个阈值（比如 0.01 或 0.02），去掉低于阈值的特征，重新训练模型。
其实不用手动删特征，XGBoost/LightGBM 自带正则化，会自动降低弱特征的权重。
手动删特征主要是为了减少计算量和提高可解释性。
comment:Scoring
...
comment:目前的实现中，每个目标都重新提取一次特征，这非常低效：
Dry_Green_g: 提取357张图片的特征
Dry_Dead_g: 再次提取357张图片的特征
Dry_Clover_g: 第三次提取357张图片的特征
...
总共提取了 357 × 5 = 1785 次！
但你看到的 Processing: 1700/357 这样的奇怪数字，是因为 target_df 包含了所有行（1785行），但实际只有357张唯一图片。
应该怎么做？
特征应该只计算一次，然后复用到所有5个目标

comment:提取所有 5 个目标中重要性分数大于 $0.02$ 的特征，并找出这些特征集合的并集（Union）
将上述所有特征集合进行合并，得到最终的并集特征列表（共 21 个唯一特征）。类别并集特征列表 (共 21 个特征)
几何/基本Height_cm
日期/状态State, Month, Species
颜色指数 (ExG)ExG_median, ExG_mean, ExG_hist_9, ExG_hist_8, ExG_hist_7, ExG_hist_5, ExG_hist_1, ExG_hist_0
颜色指数 (ExR)ExR_mean, ExR_median, ExR_hist_9, ExR_hist_4
颜色指数 (其它)CIVE_mean, NDVI, VARI_mean
HSV 特征HSV_S_std, HSV_S_hist_4, HSV_H_hist_7, HSV_H_hist_5, HSV_H_hist_4
GLCM 纹理GLCM_homogeneity_mean, GLCM_correlation_std
最终特征集合（按字母顺序）：CIVE_mean, ExG_hist_0, ExG_hist_1, ExG_hist_5, ExG_hist_7, ExG_hist_8, ExG_hist_9, 
ExG_mean, ExG_median, ExR_hist_4, ExR_hist_9, ExR_mean, ExR_median, GLCM_correlation_std, GLCM_homogeneity_mean,
HSV_H_hist_4, HSV_H_hist_5, HSV_H_hist_7, HSV_S_hist_4, HSV_S_std, Height_cm, Month, NDVI, Species, State, VARI_mean

comment:报错如下
---------------------------------------------------------------------------
KeyError                                  Traceback (most recent call last)
/tmp/ipykernel_47/166374653.py in <cell line: 0>()
    389 
    390     # Predict test set
--> 391     submission = predictor.predict('/kaggle/input/csiro-biomass/test.csv', image_dir='/kaggle/input/csiro-biomass/test')
    392 
    393     # Save submission file

/tmp/ipykernel_47/166374653.py in predict(self, test_csv_path, image_dir)
    342 
    343         # Extract features for each image (only once)
--> 344         X_test = self.prepare_features(unique_images, is_training=False)
    345 
    346         # Predict for each target

/tmp/ipykernel_47/166374653.py in prepare_features(self, df, is_training)
    240             self.feature_names = feature_df.columns.tolist()
    241 
--> 242         return feature_df[self.feature_names].values
    243 
    244     def train(self, train_csv_path, image_dir='/kaggle/input/csiro-biomass/train'):
...
报错如上
这个错误是因为测试集CSV中没有 State, Species, Sampling_Date 等元数据列，但训练时使用了这些特征。
我需要修复 prepare_features 函数来处理这种情况
